...
export abstract class BaseDatabase implements DatabaseInterface {
  ...
  async runMigrations(migrations: Migration[]): Promise<void> {
    ...
    this.logger.info(
      `Running ${pendingMigrations.length} database migrations...`
    );
    ...
    for (const migration of pendingMigrations.sort(
      ...
      try {
        ...
        this.logger.error(
          `Migration ${migration.version} failed: ${error instanceof Error ? error.message : error}`
        );
        throw error;
      }
    }
  ...
  abstract createRoomMapping(
    mapping: Omit<RoomMappingRow, 'id'>
  ): Promise<RoomMappingRow>;
  abstract getRoomMappingByRoomId(
    matrixRoomId: string
  ): Promise<RoomMappingRow | null>;
  abstract getRoomMappingsForPackage(
    packageName: string
  ): Promise<RoomMappingRow[]>;
  abstract updateRoomMapping(
    id: number,
    updates: Partial<RoomMappingRow>
  ): Promise<void>;
  ...
  abstract createMessageMapping(
    mapping: Omit<MessageMappingRow, 'id'>
  ): Promise<MessageMappingRow>;
  abstract getMessageMappingByEventId(
    matrixEventId: string
  ): Promise<MessageMappingRow | null>;
  abstract getMessageMappingsByReviewId(
    reviewId: string
  ): Promise<MessageMappingRow[]>;
  ...
  abstract getGooglePlayReview(
    reviewId: string
  ): Promise<GooglePlayReviewRow | null>;
  abstract updateGooglePlayReview(
    reviewId: string,
    updates: Partial<GooglePlayReviewRow>
  ): Promise<void>;
  abstract getReviewsForPackage(
    packageName: string,
    limit?: number
  ): Promise<GooglePlayReviewRow[]>;
  abstract getReviewsModifiedSince(
    since: Date,
    packageName?: string
  ): Promise<GooglePlayReviewRow[]>;
  ...
  abstract getMatrixMessagesForRoom(
    roomId: string,
    limit?: number
  ): Promise<MatrixMessageRow[]>;
