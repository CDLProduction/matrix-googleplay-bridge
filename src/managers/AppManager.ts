import { GooglePlayApp, BridgeConfig } from '../models/ConfigTypes';
...
export class AppManager extends EventEmitter {
  ...
  public async addApp(app: GooglePlayApp): Promise<void> {
    ...
    this.logger.info(
      `Added app: ${app.packageName} (${app.appName || 'Unnamed'})`
    );
  ...
  public getAllApps(): GooglePlayApp[] {
    return Array.from(this.apps.values()).map(context => context.app);
  }
  ...
  public updateAppStats(
    packageName: string,
    updates: Partial<AppContext['stats']>
  ): void {
    const context = this.apps.get(packageName);
    if (context) {
  ...
  public parseUserNamespace(
    matrixUserId: string
  ): { packageName: string; userId: string } | null {
  ...
  private async setupAppRoom(context: AppContext): Promise<void> {
    ...
    try {
      ...
      if (existingRoom && typeof existingRoom === 'string') {
        ...
        this.logger.info(
          `Using existing room ${existingRoom} for ${context.app.packageName}`
        );
        return;
      }
    } catch (error) {
    ...
    const roomOptions: any = {
      name:
        context.app.roomName ||
        `${context.app.appName || context.app.packageName} Reviews`,
      topic:
        context.app.roomTopic ||
        `Google Play reviews for ${context.app.appName || context.app.packageName}`,
    ...
    try {
      ...
      const roomId =
        typeof roomResult === 'string' ? roomResult : roomResult.room_id;
      ...
      this.logger.error(
        `Failed to create room for ${context.app.packageName}:`,
        error
      );
      throw error;
    }
  ...
  private async inviteRoomMembers(
    roomId: string,
    app: GooglePlayApp
  ): Promise<void> {
    const intent = this.bridge.getIntent();
    ...
    if (app.admins) {
      for (const admin of app.admins) {
        try {
          ...
          this.logger.warn(
            `Failed to invite admin ${admin} to room ${roomId}:`,
            error
          );
        }
      }
    ...
    if (app.moderators) {
      for (const moderator of app.moderators) {
        try {
          ...
          this.logger.warn(
            `Failed to invite moderator ${moderator} to room ${roomId}:`,
            error
          );
        }
      }
  ...
  public async reloadConfiguration(): Promise<void> {
    ...
    try {
      ...
      const newApps = new Set(
        newConfig.googleplay.applications.map(app => app.packageName)
      );
      ...
      this.logger.error('Failed to reload app manager configuration', {
        error,
      });
      throw error;
    }
